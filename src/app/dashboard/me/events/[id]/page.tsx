...
const fetchEvent = async (id: string): Promise<Event> => {
  try {
    const response = await axiosInstance.get(`/api/proxy/events/me/${id}`);
...
export default function Dashboard() {
  const { events: eventRecommendations, user: currentUser } = useSelector(
    (state: RootState) => ({
  ...
  const {
    ...
    queryKey: ["events", currentUser?.profile.id, id],
